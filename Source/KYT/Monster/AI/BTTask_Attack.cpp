	...
	UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	int32 AttackAnimNum = FMath::RandRange(1, 4);
	switch (AttackAnimNum) {

	case 1 :
		Anim->ChangeAnim(EMonsterAnimType::Attack1);
		break;
	case 2:
		Anim->ChangeAnim(EMonsterAnimType::Attack2);
		break;
	case 3:
		Anim->ChangeAnim(EMonsterAnimType::Attack3);
		break;
	case 4:
		Anim->ChangeAnim(EMonsterAnimType::Attack4);
		break;
	...
	float DeltaSeconds)
{
	...
	if (Monster->GetAttackEnd())
	{
		...
		// 타겟과 몬스터와의 거리를 체크한다.
		const FMonsterBaseInfo& Info = Monster->GetMonsterInfo();
